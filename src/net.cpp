...
static const char *strDNSSeed[][2] = {
    {"tittycoin.org", "dnsseed.tittycoin.org"},
    {"tittyco.in", "dnsseed.tittyco.in"}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TittyCoin is probably already running."), addrBind.ToString().c_str());
        else
